...
link_1 = p.createMultiBody(
    ...
    basePosition=[0, 0, link_1_length / 2]
)
...
link_2 = p.createMultiBody(
    ...
    basePosition=[link_1_length, 0, link_2_length / 2]
)
...
joint_1 = p.createConstraint(
    ...
    jointType=p.JOINT_REVOLUTE, jointAxis=[0, 0, 1], 
    parentFramePosition=[link_1_length, 0, 0], childFramePosition=[0, 0, 0]
)
...
p.setJointMotorControlArray(
    ...
    [0, 1],  # Joint indices
    p.POSITION_CONTROL, 
    targetPositions=joint_angles[:2]  # First two joints
)

# Run the simulation to see the robot move
for _ in range(240):
    p.stepSimulation()
    time.sleep(1./240.)
